...
const RecruitNewGroup = () => {
  ...
  const same_group = async ()=>{
    setError(null); //에러 초기화
      ...
      try{
        const response = await axios.get('http://localhost:3000/api/group/group_namecheck',{
  ...
  const handlefinishbutton  = async ()=>{
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/group/participation', {  // 서버의 회원가입 endpoint로 POST 요청
      group_name : group_name,
      ...
      if (response.data) {  // 응답 데이터가 있다면 회원가입이 성공한 것으로 간주
        ...
        localStorage.setItem('group_name',group_name); //모임이름 로컬 스토리지에 저장 1
        console.log('모임생성 성공:', response.data);
        return true;
      }
    }
    catch(err){ //오류 발생시 
  ...
  return (
    <div className={styles.recruitNewGroup}>
      <img
        className={styles.newGroupBincani}
        alt=""
        src="/bincani.gif"
      />
      ...
      {/*동일 아이디 체크 버튼*/}
      <img className={styles.yesButton2Icon} alt="" src="/yes-button2.svg" onClick={same_group} />
     
      {/*암호 동일한지 체크*/}
      <img className={styles.yesButton2Icon1} alt="" src="/yes-button2.svg" onClick={handleYesButton}  />
