...
const Join = () => {
  ...
  const handleJoin = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/user/join', {  // 서버의 회원가입 endpoint로 POST 요청
        user_id: user_id,
        password: password,
        email: email,
        username: username,
  ...
  const same_ID = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/user/join_idcheck', {  // 서버의 회원가입 endpoint로 POST 요청
      user_id: user_id
      });
    }
    catch (err) {  // 오류 발생 시
  ...
  const same_email = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/user/join_emailcheck', {  // 서버의 회원가입 endpoint로 POST 요청
      email: email
      });
    }
    catch (err) {  // 오류 발생 시
...
<form>
        ...
        <img className={styles.yesButtonIcon2} alt="" src="/yes-button.svg" onClick={handleYesButtonClick}/>
